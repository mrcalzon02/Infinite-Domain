...
Status: **Every structure-template-feasible neutral feature is implemented: the five original optional environmental structures, the full AGE-001–015 environmental/geology family, and 44 of the 64 OSF-* neutral pool IDs (see `tools/abyssal_worldgen/abyssal_feature_catalog.json`, 60 tracked entries) — systemic abyssal deformation and custom deep cave carvers remain active / runtime appearance unmeasured. Both factional oceanographic-remnant spawn pools (AGE-016 Pelagos, AGE-017 Karsic) are complete at their full 24-entry PEL-DET/KAR-DET catalogs plus AGE-006–010 folded in (see `tools/abyssal_worldgen/abyssal_factional_debris_catalog.json`, 57 tracked entries). The Akula submarine (`docs/deep-sea-structures.md`) is live as an Eastern/Karsic landmark. Remaining backlog is exclusively terrain-scale/density-function work outside this pipeline's reach: 12 OSF landform IDs (seamounts, calderas, canyons, graben fields, fault scarps, contourite drifts, trench axial channels) and the 8-ID OSF-057–064 ambient microfeature family — see `docs/ABYSSAL_NEUTRAL_SEAFLOOR_FEATURE_POOL.md` for the exact breakdown.**
