#!/usr/bin/env python3
"""[SYSTEM REPORT] OWS-004 Gate-D final authoritative review.

Consumes the production OWS-004 dispatch, applies the same door stabilization as
normal generation, requires exact decompressed-NBT equality with the generated
shipping structure, and renders only that shipping NBT. This proves mechanical
synchronization only; manual Gate-D visual approval remains mandatory.
"""
...
import old_world_ows004_final as final_builder
...
OUTPUT_DIR = OUTPUT_ROOT / "OWS-004" / "gate_d_final" / "r1"
TARGET = "OWS-004"
SPEC = core.BY_TARGET[TARGET]
SHIPPING_NBT = ROOT / "kubejs" / "data" / "infinite_domain" / "structure" / "wasteland" / "old_world" / f"{SPEC.name}.nbt"
TEMP_NAME = "old_world/_heavy_review_ows004_gate_d_sync_r1"
TEMP_NBT = ROOT / "kubejs" / "data" / "infinite_domain" / "structure" / "wasteland" / "old_world" / "_heavy_review_ows004_gate_d_sync_r1.nbt"
...
def _assert_authoritative_dispatch() -> None:
    if core.BUILDERS.get(TARGET) is not final_builder.build_004:
        raise AssertionError(
            "OWS-004 authoritative dispatch no longer points directly to "
            "old_world_ows004_final.build_004"
        )
...
def _serialize_authoritative_builder() -> tuple[str, str, int]:
    ...
    try:
        built = _raw_nbt(TEMP_NBT)
        shipping = _raw_nbt(SHIPPING_NBT)
        if built != shipping:
            raise AssertionError(
                "OWS-004 synchronization failure: authoritative serialization does not exactly match shipping NBT. "
                f"builder_sha256={_sha256(built)} shipping_sha256={_sha256(shipping)}"
            )
        return _sha256(built), _sha256(shipping), len(built)
    finally:
...
def main() -> None:
    ...
    if state.get("active_target") != TARGET:
        print(f"Gate-D OWS-004 renderer skipped: active target is {state.get('active_target')}")
        return

    gate_c = state.get("visual_review_gates", {}).get("gate_c_damage_states", {})
    if gate_c.get("status") != "passed_r4":
        print(f"Gate-D OWS-004 renderer skipped: Gate C status={gate_c.get('status')}")
        return
    ...
    if status not in allowed:
        print(f"Gate-D OWS-004 renderer skipped: status={status}")
        return

    builder_hash, shipping_hash, raw_size = _serialize_authoritative_builder()
    size, blocks = unpack_structure(SHIPPING_NBT)
    if tuple(size) != (51, 47, 47):
        raise AssertionError(f"OWS-004 final shipping dimensions changed unexpectedly: {size}")
    ...
    sync_record = {
        ...
        "revision": f"gate-d-r1@{revision}",
        "spec_name": SPEC.name,
        "authoritative_dispatch": "old_world_narrative_core.BUILDERS['OWS-004']",
        "authoritative_builder": "old_world_ows004_final.build_004",
    ...
    (OUTPUT_DIR / "review_manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8", newline="\n")
    ...
    state["active_target_passes"]["visual_gate_d_final_multi_angle"] = "r1_rendered_authoritative_sync_verified_pending_manual_review"
    ...
    gate["review_stage_source"] = "scripts/render_ows004_gate_d_final.py"
    ...
    state.setdefault("planning_records", {})["pass_19_micro_detail"] = "old_world_narrative/reviews/heavy_rebuild/OWS-004_PASS19_MICRODETAIL.md"
    STATE_PATH.write_text(json.dumps(state, indent=2) + "\n", encoding="utf-8", newline="\n")

    print(f"OWS-004 Gate-D r1 exact shipping sync passed: sha256={shipping_hash}, bytes={raw_size}, size={size}.")
