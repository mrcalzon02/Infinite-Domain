#!/usr/bin/env python3
"""Image-regression guard for OWS-008 Gate-D r1; never approves the gate."""
...
BASE = ROOT / "dev/old_world_narrative" / "reviews" / "heavy_rebuild" / "visual" / "OWS-008"
...
def main() -> None:
    ...
    if not sync.get("exact_serialized_nbt_match") or not sync.get("exact_decompressed_nbt_match"):
        raise AssertionError("OWS-008 exact shipping synchronization proof is missing")
    if sync.get("render_source") != "shipping_nbt" or sync.get("fixed_camera_set") != "ows008_fixed_v1":
        raise AssertionError("OWS-008 Gate D was not rendered from shipping NBT with the frozen camera set")
    if sync.get("pass19_positions_verified") != 8 or sync.get("canonical_proof_nodes") != 1:
        raise AssertionError("OWS-008 Pass-19/proof shipping evidence is incomplete")
    if sync.get("deterministic_spawners") != 3 or not sync.get("protected_routes_clear"):
        raise AssertionError("OWS-008 encounter/route shipping evidence is incomplete")
    ...
    report = {
        ...
        "checks": {
            ...
            "pass19_proof_encounters_routes_and_required_blocks_retained": True,
        },
    ...
    print(f"OWS-008 Gate-D regression passed: average={average:.4f}, exterior={exterior_average:.4f}, max={maximum:.4f}")
