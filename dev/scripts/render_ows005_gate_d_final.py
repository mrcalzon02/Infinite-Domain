#!/usr/bin/env python3
"""Prepare OWS-005 Gate-D r1 from the actual stabilized shipping NBT.

This target-local tool writes review artifacts only. It never updates shared
state, dispatch, registries, generated shipping structures, or gate decisions.
"""
...
import old_world_ows005_final as final_builder
...
TEMP_NAME = "old_world/_heavy_review_ows005_gate_d_sync_r1"
TEMP_NBT = ROOT / "kubejs" / "data" / "infinite_domain" / "structure" / "wasteland" / "old_world" / "_heavy_review_ows005_gate_d_sync_r1.nbt"
...
def _sync() -> dict[str, object]:
    if generation.BUILDERS.get(TARGET) is not final_builder.build_005:
        raise AssertionError("OWS-005 production dispatch is not old_world_ows005_final.build_005")
    # Production generation uses the repository's canonical `python` runtime.
    # Invoke that exact serialization path so gzip framing as well as the
    # decompressed NBT payload can be compared byte-for-byte with shipping.
    probe = (
        "import sys;sys.path.insert(0,'scripts');"
        "import generate_old_world_narrative_structures as g,old_world_ows005_final as f;"
        "assert g.BUILDERS['OWS-005'] is f.build_005;"
        "t=g.BUILDERS['OWS-005']();g.base.stabilize_door_pairs(t);"
    ...
    try:
        ...
        if built_bytes != shipping_bytes or built_raw != shipping_raw:
            raise AssertionError(
                "OWS-005 stabilized builder/shipping mismatch: "
        ...
        if shipping_hash != EXPECTED_SHIPPING_SHA256:
            raise AssertionError(f"OWS-005 shipping hash drift: {shipping_hash} != {EXPECTED_SHIPPING_SHA256}")
        return {
...
def main() -> None:
    ...
    if state.get("active_target") != TARGET:
        raise AssertionError(f"OWS-005 is not active: {state.get('active_target')}")
    if state.get("visual_review_gates", {}).get("gate_c_damage_states", {}).get("status") != "passed_r1":
        raise AssertionError("OWS-005 Gate C r1 is not passed")

    sync = _sync()
    size, blocks = unpack_structure(SHIPPING_NBT)
    if tuple(size) != (59, 24, 51):
        raise AssertionError(f"OWS-005 shipping dimensions changed: {size}")
    ...
    sync_record = {
        ...
        "revision": f"gate-d-r1@{revision}",
        "spec_name": SPEC.name,
        "authoritative_dispatch": "generate_old_world_narrative_structures.BUILDERS['OWS-005']",
        "authoritative_builder": "old_world_ows005_final.build_005",
        "production_door_stabilization_applied": True,
        "stabilized_door_half_positions": [[20, 3, 6], [27, 2, 38]],
    ...
    print(f"OWS-005 Gate-D r1 candidate rendered from shipping NBT: {sync['shipping_nbt_sha256']}")
