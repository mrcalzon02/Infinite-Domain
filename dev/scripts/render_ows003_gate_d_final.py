#!/usr/bin/env python3
"""[SYSTEM REPORT] OWS-003 Gate-D final authoritative review.

The renderer consumes the production OWS-003 dispatch, applies the same door
stabilization as normal generation, requires exact decompressed-NBT equality with
the freshly generated shipping structure, and renders only that shipping NBT.
"""
...
import old_world_ows003_final as final_builder
...
TEMP_NAME = "old_world/_heavy_review_ows003_gate_d_sync_r1"
TEMP_NBT = ROOT / "kubejs" / "data" / "infinite_domain" / "structure" / "wasteland" / "old_world" / "_heavy_review_ows003_gate_d_sync_r1.nbt"
...
def _assert_authoritative_dispatch() -> None:
    if core.BUILDERS.get(TARGET) is not final_builder.build_003:
        raise AssertionError(
            "OWS-003 authoritative dispatch no longer points directly to "
            "old_world_ows003_final.build_003"
        )
...
def _serialize_authoritative_builder() -> tuple[str, str, int]:
    ...
    try:
        ...
        if built != shipping:
            raise AssertionError(
                "OWS-003 synchronization failure: authoritative serialization does not exactly match shipping NBT. "
...
def main() -> None:
    ...
    if state.get("active_target") != TARGET:
        print(f"Gate-D OWS-003 renderer skipped: active target is {state.get('active_target')}")
        return

    gate_c = state.get("visual_review_gates", {}).get("gate_c_damage_states", {})
    if gate_c.get("status") != "passed_r3":
        print(f"Gate-D OWS-003 renderer skipped: Gate C status={gate_c.get('status')}")
        return
    ...
    if status not in allowed:
        print(f"Gate-D OWS-003 renderer skipped: status={status}")
        return

    builder_hash, shipping_hash, raw_size = _serialize_authoritative_builder()
    size, blocks = unpack_structure(SHIPPING_NBT)
    if tuple(size) != (59, 24, 51):
        raise AssertionError(f"OWS-003 final shipping dimensions changed unexpectedly: {size}")
    ...
    sync_record = {
        ...
        "revision": f"gate-d-r1@{revision}",
        "spec_name": SPEC.name,
        "authoritative_dispatch": "old_world_narrative_core.BUILDERS['OWS-003']",
        "authoritative_builder": "old_world_ows003_final.build_003",
    ...
    gate["review_stage_source"] = "scripts/render_ows003_gate_d_final.py"
    ...
    state.setdefault("planning_records", {})["pass_19_micro_detail"] = "old_world_narrative/reviews/heavy_rebuild/OWS-003_PASS19_MICRODETAIL.md"
    STATE_PATH.write_text(json.dumps(state, indent=2) + "\n", encoding="utf-8", newline="\n")

    print(f"OWS-003 Gate-D r1 exact shipping sync passed: sha256={shipping_hash}, bytes={raw_size}, size={size}.")
