...
def _vertical_cores_and_services(t: base.Template) -> None:
    ...
    # Build the shaft and landings first; install the ladder last so no landing
    # floor can overwrite the continuous vertical climb column.
...
def main() -> None:
    ...
    for key in ("structural_system", "circulation_and_access", "exterior_architecture", "interior_architecture", "operational_systems", "institutional_identity"):
    ...
    print(f"Rendered OWS-004 Gate B r1 intact review at {manifest['dimensions']} using {manifest['fixed_camera_set']}; manual intact-state approval remains pending.")
