...
$project = Join-Path $root 'packdev\unified-radiation'
...
$output = Join-Path $mods 'infinite-domain-unified-radiation-1.0.0.jar'
...
$dependencyPaths = @(
    ...
    (Join-Path $mods 'wastelands-2.4.0-neoforge.1.jar'),
    (Join-Path $mods 'the_wasteland_reworked-0.6.0-neoforge-1.21.1.jar')
)
...
foreach ($old in Get-ChildItem -LiteralPath $mods -Filter 'infinite-domain-unified-radiation-*.jar') {
