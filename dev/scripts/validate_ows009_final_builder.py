#!/usr/bin/env python3
"""Target-local mechanical validation for the pure OWS-009 final builder."""
...
import old_world_ows009_final as final
...
def main() -> None:
    source_path = ROOT / "dev/scripts/old_world_ows009_final.py"
    ...
    accepted_hash = serialize_hash(accepted, "_validate_ows009_accepted_d3")
    if accepted_hash != final.ACCEPTED_GATE_C_D3_SHA256:
        raise AssertionError(f"pure accepted-D3 reproduction drifted: {accepted_hash} != {final.ACCEPTED_GATE_C_D3_SHA256}")

    built = final.build_009()
    final._assert_final_contracts(built)
    raw_hash = serialize_hash(built, "_validate_ows009_final_raw")
    ...
    if final._count(accepted, "create:andesite_casing") != 0:
        raise AssertionError("accepted D3 unexpectedly contains create:andesite_casing")
    if final._count(built, "create:andesite_casing") != 1:
        raise AssertionError("final builder must serialize exactly one create:andesite_casing")
    modded_additions = {pos: block for pos, block in final.PASS19_MICRODETAIL.items() if not block.startswith("minecraft:")}
    if modded_additions != {(34, 2, 28): "create:andesite_casing"}:
        raise AssertionError(f"unexpected modded Pass-19 additions: {modded_additions}")

    stabilized = final.build_009()
    ...
    stabilized_hash = serialize_hash(stabilized, "_validate_ows009_final_stabilized")
    metrics = base.fidelity_metrics(stabilized)
    lint = base.assess_fidelity("service_garage", stabilized)
    if metrics["orphan_door_halves"] != 0 or metrics["working_doors"] != 19:
        raise AssertionError(f"OWS-009 door lint drifted: {metrics}")
    if not lint["structural_lint_passed"] or lint["issues"]:
        raise AssertionError(f"OWS-009 structural lint failed: {lint}")
    ...
    if any(not (0 <= x < 49 and 0 <= y < 18 and 0 <= z < 41) for x, y, z in stabilized.blocks):
        raise AssertionError("stabilized OWS-009 exceeds accepted bounds")

    print("OWS-009 pure final builder validation passed")
    print(f"accepted_d3_sha256={accepted_hash}")
    print(f"final_raw_sha256={raw_hash}")
    print(f"final_stabilized_sha256={stabilized_hash}")
    print(f"pass19_additions={len(delta)} replacements=0")
    print(f"stabilization_delta={len(stabilization_delta)}")
    print(f"source_sha256={hashlib.sha256(source_path.read_bytes()).hexdigest()}")
    print(f"metrics={metrics}")
    print(f"structural_lint={lint}")
    print("required_blocks=" + ",".join(f"{block}:{final._count(stabilized, block)}" for block in final.PRODUCTION_REQUIRED_BLOCKS))
