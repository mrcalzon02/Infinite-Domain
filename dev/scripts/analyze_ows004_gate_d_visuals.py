#!/usr/bin/env python3
"""[SYSTEM REPORT] Image-level regression guard for synchronized OWS-004 Gate D.

Compares final shipping-NBT renders with the manually approved Gate-C r4 D3 set.
This check may reject reconstruction drift but cannot approve Gate D visually.
"""
...
BASE = ROOT / "dev/old_world_narrative" / "reviews" / "heavy_rebuild" / "visual" / "OWS-004"
GATE_C = BASE / "gate_c_damage_states" / "r4" / "d3"
...
def main() -> None:
    ...
    if state.get("active_target") != "OWS-004":
        print(f"OWS-004 Gate-D visual regression skipped: active target is {state.get('active_target')}")
        return
    ...
    if gate.get("status") not in {
        ...
        print(f"OWS-004 Gate-D visual regression skipped: status={gate.get('status')}")
        return

    sync = json.loads(SYNC_PATH.read_text(encoding="utf-8"))
    if not sync.get("exact_decompressed_nbt_match") or sync.get("render_source") != "shipping_nbt":
        raise AssertionError("OWS-004 Gate-D exact shipping synchronization proof is missing")
    ...
    for view in VIEWS:
        ...
        try:
            ...
            if baseline == 0:
                raise AssertionError(f"Gate-C r4 {view} contains no foreground pixels")
    ...
    if average > 0.08:
        raise AssertionError(f"OWS-004 Gate-D drift too large for Pass-19 microdetail: average={average:.4f}")
    if maximum > 0.16:
        raise AssertionError(f"OWS-004 Gate-D single-view drift too large: max={maximum:.4f}")
    for view in EXTERIOR:
        if not 0.95 <= silhouette[view] <= 1.05:
            raise AssertionError(f"OWS-004 Gate-D exterior silhouette drifted in {view}: {silhouette[view]:.4f}")
    if not 0.90 <= silhouette["roof_top_oblique"] <= 1.10:
        raise AssertionError(f"OWS-004 Gate-D roof/site silhouette drifted: {silhouette['roof_top_oblique']:.4f}")

    report = {
        "target": "OWS-004",
        "gate": "gate_d_final",
        "revision": "r1",
        "comparison": "Gate-D authoritative shipping NBT renders vs Gate-C r4 approved D3 renders",
        "fixed_camera_set": gate.get("fixed_camera_set", "ows004_fixed_v1"),
        "authoritative_sync": {
            "exact_decompressed_nbt_match": True,
            "shipping_nbt_sha256": sync.get("shipping_nbt_sha256"),
            "render_source": "shipping_nbt",
        },
        "visible_change_ratio_from_gate_c_r4_d3": changed,
        "foreground_silhouette_ratio_from_gate_c_r4_d3": silhouette,
    ...
    print(f"OWS-004 Gate-D image regression passed: average={average:.4f}, exterior={exterior_average:.4f}, max={maximum:.4f}.")
