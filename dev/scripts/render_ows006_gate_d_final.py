#!/usr/bin/env python3
"""Prepare OWS-006 Gate-D r1 from the actual stabilized shipping NBT.
...
import old_world_ows006_final as final_builder
...
TARGET = "OWS-006"
EXPECTED_SHIPPING_SHA256 = "442b097668422975dd6f2fb663526b47d689b8b76c48faa5e2463bc8c6212fbb"
...
TEMP_NAME = "old_world/_heavy_review_ows006_gate_d_sync_r1"
TEMP_NBT = ROOT / "kubejs" / "data" / "infinite_domain" / "structure" / "wasteland" / "old_world" / "_heavy_review_ows006_gate_d_sync_r1.nbt"
...
def _sync() -> dict[str, object]:
    if generation.BUILDERS.get(TARGET) is not final_builder.build_006:
        raise AssertionError("OWS-006 production dispatch is not old_world_ows006_final.build_006")

    # Execute the exact normal dispatch/stabilization/save path using the same
    # canonical production Python runtime used for authoritative generation.
    probe = (
        "import sys;sys.path.insert(0,'scripts');"
        "import generate_old_world_narrative_structures as g,old_world_ows006_final as f;"
        "assert g.BUILDERS['OWS-006'] is f.build_006;"
        "t=g.BUILDERS['OWS-006']();g.base.stabilize_door_pairs(t);"
    ...
    try:
        ...
        if built_bytes != shipping_bytes or built_raw != shipping_raw:
            raise AssertionError(
                "OWS-006 stabilized builder/shipping mismatch: "
        ...
        if shipping_hash != EXPECTED_SHIPPING_SHA256:
            raise AssertionError(f"OWS-006 shipping hash drift: {shipping_hash} != {EXPECTED_SHIPPING_SHA256}")
        return {
            ...
            "decompressed_nbt_sha256": _sha(shipping_raw),
...
def main() -> None:
    ...
    if state.get("active_target") != TARGET:
        raise AssertionError(f"OWS-006 is not active: {state.get('active_target')}")
    if state.get("visual_review_gates", {}).get("gate_c_damage_states", {}).get("status") != "passed_r2":
        raise AssertionError("OWS-006 Gate C r2 is not passed")

    sync = _sync()
    size, blocks = unpack_structure(SHIPPING_NBT)
    if tuple(size) != (59, 26, 51):
        raise AssertionError(f"OWS-006 shipping dimensions changed: {size}")
    ...
    sync_record = {
        ...
        "revision": f"gate-d-r1@{revision}",
        "spec_name": SPEC.name,
        "authoritative_dispatch": "generate_old_world_narrative_structures.BUILDERS['OWS-006']",
        "authoritative_builder": "old_world_ows006_final.build_006",
        "normal_generation_stabilization": "generate_wasteland_sites.stabilize_door_pairs",
        "production_door_stabilization_applied": True,
        "stabilized_door_half_positions": [],
    ...
    print(f"OWS-006 Gate-D r1 candidate rendered from shipping NBT: {sync['shipping_nbt_sha256']}")
