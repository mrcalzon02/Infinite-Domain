#!/usr/bin/env python3
"""[SYSTEM REPORT] Image-level regression guard for synchronized OWS-001 Gate D.

Gate C already approved the D3 historical/damage composition. Gate D adds only
Pass-19 microdetail and must render the authoritative shipping NBT. This analyzer
therefore compares the actual persisted Gate-D PNGs against the approved Gate-C
D3 PNGs using the same camera set. It rejects gross visual drift or silhouette
changes while leaving final human/visual approval as a separate blocking gate.
"""
...
BASE = ROOT / "dev/old_world_narrative" / "reviews" / "heavy_rebuild" / "visual" / "OWS-001"
...
def main() -> None:
    ...
    if state.get("active_target") != "OWS-001":
        print(f"Gate-D visual regression skipped: active target is {state.get('active_target')}")
        return
    ...
    if gate.get("status") not in {
        ...
        print(f"Gate-D visual regression skipped: status={gate.get('status')}")
        return

    sync = json.loads(SYNC_PATH.read_text(encoding="utf-8"))
    if not sync.get("exact_decompressed_nbt_match"):
        raise AssertionError("Gate-D authoritative synchronization proof is not exact")
    if sync.get("render_source") != "shipping_nbt":
        raise AssertionError("Gate-D was not rendered from the shipping NBT")
    ...
    for view in VIEWS:
        ...
        try:
            ...
            if baseline_foreground == 0:
                raise AssertionError(f"Gate-C {view} contains no foreground pixels")
    ...
    # Pass 19 is explicitly final microdetail. The final shipping structure may
    # visibly change, especially in roof/interior views, but it may not become a
    # second architectural/damage rebuild after Gate C was approved.
    if average_change > 0.08:
        raise AssertionError(f"Gate-D drift is too large for microdetail: average={average_change:.4f}")
    if maximum_change > 0.16:
        raise AssertionError(f"one Gate-D camera changed too much after Gate C: max={maximum_change:.4f}")
    for view in EXTERIOR:
        ratio = silhouette[view]
        if not 0.95 <= ratio <= 1.05:
            raise AssertionError(f"Gate-D exterior silhouette drifted in {view}: {ratio:.4f}")
    if not 0.90 <= silhouette["roof_top_oblique"] <= 1.10:
        raise AssertionError(
            "Gate-D roof/site silhouette changed beyond microdetail range: "
    ...
    report = {
        ...
        "gate": "gate_d_final",
        "revision": "r1",
        "comparison": "Gate-D authoritative shipping NBT renders vs Gate-C approved D3 renders",
        "fixed_camera_set": gate.get("fixed_camera_set", "ows001_fixed_v1"),
        "authoritative_sync": {
            "exact_decompressed_nbt_match": True,
            "shipping_nbt_sha256": sync.get("shipping_nbt_sha256"),
            "render_source": "shipping_nbt",
        },
        "visible_change_ratio_from_gate_c_d3": changed,
        "foreground_silhouette_ratio_from_gate_c_d3": silhouette,
        "averages": {
        ...
        "note": "This is a blocking regression guard, not an automatic Gate-D visual approval.",
    }
    ...
    print(
        "OWS-001 Gate-D image regression checks passed: "
