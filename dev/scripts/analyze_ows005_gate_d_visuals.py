#!/usr/bin/env python3
"""Image-regression guard for OWS-005 Gate-D r1; never approves the gate."""
...
BASE = ROOT / "dev/old_world_narrative" / "reviews" / "heavy_rebuild" / "visual" / "OWS-005"
...
def main() -> None:
    ...
    if not sync.get("exact_serialized_nbt_match") or not sync.get("exact_decompressed_nbt_match"):
        raise AssertionError("OWS-005 exact shipping synchronization proof is missing")
    if sync.get("render_source") != "shipping_nbt":
        raise AssertionError("OWS-005 Gate D was not rendered from shipping NBT")
    ...
    print(f"OWS-005 Gate-D regression passed: average={average:.4f}, exterior={exterior_average:.4f}, max={maximum:.4f}")
