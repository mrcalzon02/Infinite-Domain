"""Rebuild dev/docs/MOD_LIST.md, dev/docs/registry-inventory/mod-jar-index.json,
and dev/docs/registry-inventory/entity-ids.txt from the jars actually present in mods/.
...
MODS = ROOT / "mods"
REGISTRY_DIR = ROOT / "dev/docs" / "registry-inventory"

# Project-built jars -> their tracked source directory under packdev/.
PROJECT_SOURCES = {
    "infinite-domain-create-nuclear-balance-1.0.0.jar": "dev/packdev/create-nuclear-balance",
    "infinite-domain-cyberware-mastery-1.0.0.jar": "dev/packdev/cyberware-mastery-expansion",
    "infinite-domain-darknet-worldgen-1.8.0.jar": "dev/packdev/darknet-worldgen-patch",
    "infinite-domain-echo-economy-1.0.0.jar": "dev/packdev/echo-numismatics-bridge",
    "infinite-domain-hive-world-companion-0.1.0.jar": "dev/packdev/hive-world-companion",
    "infinite-domain-lostcities-highway-compat-1.0.0.jar": "dev/packdev/lostcities-highway-compat",
    # Not yet moved under dev/ with the rest — root packdev/ still holds its tracked sources.
    "infinite-domain-overworld-terrain-1.0.0.jar": "packdev/overworld-terrain-companion",
    "infinite-domain-stellaris-industry-1.0.0.jar": "dev/packdev/stellaris-space-industry",
    "infinite-domain-unified-radiation-1.0.0.jar": "dev/packdev/unified-radiation",
    "infinite-domain-wasteland-hex-caves-1.0.0.jar": "dev/packdev/wasteland-hex-caves",
}
...
def main():
    ...
    lines = [
        ...
        "",
        f"*Generated by `dev/scripts/build_mod_index.py` from `mods/*.jar` (reading each jar's "
        ...
        "",
        "Item/block counts (where available) are from `dev/docs/registry-inventory/namespace-summary.csv`, "
    ...
    lines += ["", f"## Project-built mods ({len(PROJECT_SOURCES)})", "",
              ...
              "CurseForge, sources tracked under `dev/packdev/`:", ""]
    ...
    (ROOT / "dev/docs" / "MOD_LIST.md").write_text("\n".join(lines) + "\n", encoding="utf-8")

    print(f"jars: {len(jar_records)}")
    print(f"entity ids: {entity_count} across {len(all_entities)} namespaces")
    print("wrote dev/docs/MOD_LIST.md, dev/docs/registry-inventory/mod-jar-index.json, "
          "dev/docs/registry-inventory/entity-ids.txt")
