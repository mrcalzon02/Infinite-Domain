...
$project = Join-Path $root 'packdev\darknet-worldgen-patch'
...
$output = Join-Path $mods 'infinite-domain-darknet-worldgen-1.8.0.jar'
...
$iceAndFire = Get-ChildItem -LiteralPath $mods -Filter 'iceandfire*.jar' | Select-Object -First 1 -ExpandProperty FullName
$javac = 'C:\Program Files\Pylo\MCreator\jdk\bin\javac.exe'
$jar = 'C:\Program Files\Pylo\MCreator\jdk\bin\jar.exe'

foreach ($required in @($minecraft, $loader, $eventBus, $brigadier, $dataFixer, $distMarker, $neoForge, $mixin, $iceAndFire, $javac, $jar)) {
...
$localClasspath = @()
foreach ($dependency in @($minecraft, $loader, $eventBus, $brigadier, $dataFixer, $distMarker, $neoForge, $mixin, $iceAndFire)) {
...
Copy-Item -LiteralPath (Join-Path $root 'DARKNET-ASSETS-LICENSE.md') -Destination (Join-Path $staging 'DARKNET-ASSETS-LICENSE.md') -Force

foreach ($oldPatch in Get-ChildItem -LiteralPath $mods -Filter 'infinite-domain-darknet-worldgen-*.jar') {
