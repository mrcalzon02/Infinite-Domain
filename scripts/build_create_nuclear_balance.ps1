...
$project = Join-Path $root 'packdev\create-nuclear-balance'
...
$output = Join-Path $mods 'infinite-domain-create-nuclear-balance-1.0.0.jar'
...
$dependencyPaths = @(
    ...
    (Join-Path $minecraftRoot 'com\google\code\gson\gson\2.10.1\gson-2.10.1.jar'),
    ...
    (Join-Path $mods 'createnuclear-1.3.2-beta.3-neoforge.jar')
)
...
foreach ($old in Get-ChildItem -LiteralPath $mods -Filter 'infinite-domain-create-nuclear-balance-*.jar') {
